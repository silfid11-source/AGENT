...
- 나레이션, 자막, 컷별 제작 지시서가 포함된 영상 제작 패키지 생성
- 품질 검사 및 자동 개선
- 분야별 에이전트 파이프라인
- 600자 초과 에이전트 결과는 핵심 전달본으로 압축해 다음 에이전트에 전달
- 앱 안에서 자동화 작업 목록을 추가하고 `workflow_inputs.csv`로 저장
- 다크 모드 UI
...
앱의 `자동화 작업 관리` 영역에서 작업을 추가하면 이 파일이 자동으로 업데이트됩니다.
...
분야별 에이전트 자동화는 결과 폴더에 다음 파일도 함께 저장합니다.

- `final_result.txt`: 최종 콘텐츠 패키지
- `agent_report.txt`: 에이전트별 작업 기록
- `video_production_package.txt`: 나레이션, 자막, 컷별 제작 지시서, 편집 메모
- `summary.txt`: 프로젝트 요약
